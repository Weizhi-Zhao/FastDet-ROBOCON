...
class CocoDetectionEvaluator():
    def __init__(self, names, anchor_sizes, device):
        self.device = device
        self.classes = []
    ...
    def coco_evaluate(self, gts, preds):
        ...
        for i, pred in enumerate(preds):
            for j in range(pred.shape[0]):
                ...
                coco_pred.dataset["annotations"].append({"image_id": i, 
                                                         "category_id": np.int32(pred[j, 0]),
                                                         "score": pred[j, 1], 
    ...
    def compute_map(self, val_dataloader, model):
        ...
        for i, (imgs, targets) in enumerate(pbar):
            ...
            with torch.no_grad():
                ...
                preds = model(imgs)
                # 特征图后处理
                output = handle_preds(preds, self.anchors, self.device, 0.001)
