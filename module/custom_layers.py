...
class DetectHead(nn.Module):
    def __init__(self, input_channels, category_num, anchor_num):
        super(DetectHead, self).__init__()
        self.conv1x1 = Conv1x1(input_channels, input_channels)

        self.obj_layers = Head(input_channels, 1 * anchor_num)
        self.reg_layers = Head(input_channels, 4 * anchor_num)
        self.cls_layers = Head(input_channels, category_num * anchor_num)
    ...
    def forward(self, x):
        ...
        reg: torch.Tensor = reg.view(-1, self.anchor_num, 4, H, W)
        cls = self.cls_layers(x)
        cls = cls.view(-1, self.anchor_num, self.category_num, H, W)
